...
<Info>
  **Inspector tabs:** - **Overview**: summary, status, size, and timing - **Headers**: request and
  response headers - **Body**: request/response bodies with format-aware viewers - **Timing**: DNS,
  connect, TLS, and transfer breakdown - **Certificate**: TLS chain details (HTTPS only)
</Info>
...
<Tip>Use copy/share actions to export the header set for debugging.</Tip>
