...
<Mermaid>
  {`sequenceDiagram
    ...
    App->>Trace: HTTP Upgrade Request
    Trace->>Server: HTTP Upgrade Request
    Server-->>Trace: 101 Switching Protocols
    Trace-->>App: 101 Switching Protocols
    
    Note over App,Server: WebSocket Connection Established
    
    App->>Trace: Text Frame (message)
    Trace->>Server: Text Frame (message)
    Server-->>Trace: Text Frame (response)
    Trace-->>App: Text Frame (response)
...
<ToggleList>
  <ToggleItem title="Debugging connection failures" defaultOpen>
  ...
  <ToggleItem title="Tracking message flow">
  ...
  <ToggleItem title="Identifying protocol violations">
