...
import { ScrollText, Plus, Wrench, Bug, ArrowRight, Tag, Code } from 'lucide-react';
...
const releases: Release[] = [
  {
    ...
    changes: [
      ...
      { category: 'added', description: 'UDP PCAP analysis and hardened flow capture lifecycle' },
      { category: 'added', description: 'Duplicate detection and editor diagnostics in the UI' },
      {
        category: 'added',
      ...
      { category: 'improved', description: 'Removed legacy protocol parsers and curl runtime' },
      {
        category: 'fixed',
      ...
      { category: 'fixed', description: 'Request duration now renders above size in list rows' },
      {
        category: 'fixed',
      ...
      { category: 'fixed', description: 'Captured bytes retained for truncated response bodies' },
      {
        category: 'fixed',
  ...
  },
  {
    ...
    changes: [
      ...
      { category: 'fixed', description: 'Stray menu dividers removed from rule list views' },
      { category: 'fixed', description: 'Toast sized to content with centered text' },
    ],
  },
  {
    version: '1.0.3',
    ...
    badge: 'Patch',
    changes: [
      { category: 'added', description: 'IPv6 support for VPN capture and VPN log export' },
      {
        category: 'added',
      ...
      { category: 'added', description: 'Script execution timeouts and improved debugging output' },
      { category: 'added', description: 'PCAP packet processing with granular error handling' },
      {
        category: 'improved',
      ...
      { category: 'fixed', description: 'Validation and pattern matching edge cases tightened' },
      {
        category: 'fixed',
      ...
      { category: 'fixed', description: 'Redirect loop prevention with bounded PCAP capture' },
      {
        category: 'fixed',
        description: 'Big-endian PCAP support with overlapping TCP segment draining',
      },
      { category: 'fixed', description: 'Filtered items now sort by timestamp' },
      { category: 'fixed', description: 'Brotli and Deflate stream end validation' },
      { category: 'fixed', description: 'Swift 6 strict concurrency compile errors resolved' },
    ],
  },
  {
    version: '1.0.1',
    ...
    badge: 'Patch',
    changes: [
      { category: 'added', description: 'Advanced capture controls with persistent settings' },
      {
        category: 'fixed',
...
function groupByCategory(changes: Change[]): Record<ChangeCategory, Change[]> {
  const groups: Record<ChangeCategory, Change[]> = { added: [], improved: [], fixed: [] };
...
export default function ChangelogPage() {
  return (
    <div className="flex w-full flex-col">
      {/* ── Left-aligned header with version badge ── */}
        ...
        <div className="mx-auto max-w-content">
          <div className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
            <div>
              ...
              <p className="mt-3 max-w-readable text-sm leading-relaxed text-muted-foreground sm:text-base">
      ...
      <EditorialBanner variant="muted">
        <h2 className="mb-3 text-2xl font-bold tracking-tight sm:text-3xl">Follow development</h2>
